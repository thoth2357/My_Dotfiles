...
toggle_mics() {
	...
	while IFS= read -r device; do
		...
		args+=(--add item mic.device.$COUNTER popup."$NAME"
			...
			label.color="$COLOR"
			click_script="SwitchAudioSource -t input -s \"${device}\" && sketchybar --set /mic.device\.*/ label.color=$COLOR_BLACK_BRIGHT --set \$NAME label.color=$COLOR_WHITE --set $NAME popup.drawing=off")
		COUNTER=$((COUNTER + 1))
...
if [ "$BUTTON" = "left" ]; then
	...
	if [[ "$MIC_NAME" != "$VALIDATED_MIC_NAME" || -z "$MIC_NAME" ]]; then
		...
		if [[ $MIC_VOLUME -lt 100 ]]; then
			...
			sketchybar -m --set mic label="$MIC_NAME 50" icon= icon.color=$COLOR_WHITE label.color=$COLOR_WHITE
		elif [[ $MIC_VOLUME -gt 0 ]]; then
			osascript -e 'set volume input volume 0'
			sketchybar -m --set mic label="$MIC_NAME 0" icon= icon.color=$COLOR_RED label.color=$COLOR_RED
		fi
	fi
